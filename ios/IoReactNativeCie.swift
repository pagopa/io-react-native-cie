...
class IoReactNativeCie: RCTEventEmitter {
  ...
  @objc func startReadingAttributes(
    ...
    Task { [weak self] in
      ...
      do {
        let atr = try await self.cieSdk.performReadAtr { event, progress in
  ...
  @objc func startReading(
    ...
    Task { [weak self] in
      ...
      do {
        let authenticatedUrl = try await self.cieSdk.performAuthentication(forUrl: authUrl, withPin: pin) { event, progress in
          let payload: NSDictionary = [ "name" : "\(event)", "progress": progress]
          self.sendEvent(withName: Event.onEvent.rawValue, body: payload)
        }
